...
def helper(input_filename, expected_filename, R, simil_threshold,
           occup_threshold,
    ...
    if actual_num_homeowners != expected_num_homeowners:
        ...
        s = s.format(actual_num_homeowners)
        s = s + "  Expected number of homeowners: {:d}\n"
        s = s.format(expected_num_homeowners)
        pytest.fail(s)
...
def test_6():
    # Check 'only one satisfactory home' relocation exception (happens
    # in Step 2)
    input_fn = "tests/a19-sample-grid.txt"
...
def test_7():
    # Check choosing among locations with same distance (happens in
    # first relocation)
